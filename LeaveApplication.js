...
const LeaveApplication = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.post('http://localhost:3001/accept-leave', emailParams);
      alert('Leave request email sent.');
    } catch (error) {
      console.error('Error sending email:', error);
      alert('Failed to send email.');
    }
    {/*try {
      const response = await axios.post('http://localhost:3001/send-leave-email', emailParams);
